...
{
    public class TokenByAdmin : IToken<TokenByAdmin>
    {
        private readonly JwtSetting jwtSettings;
        private readonly LocalDbContext db;
        private readonly IClaimsUser<ClaimsAddByAdmin> claim;

        public TokenByAdmin(JwtSetting jwtSettings, LocalDbContext db, IClaimsUser<ClaimsAddByAdmin> claim )
        {
        ...
        public bool veryfyToken(string token)
        {
            ...
            if(claims.FirstOrDefault(x => x.Type == ClaimTypes.Authentication).Value == "Activation")
            {
                return true;
            }
            return false;
